  ...
  <div style="margin-top: 70px;">
    <h3 style="font-size: 24px; margin-left: 5%">댓글</h3>
      <table class="reply-table">
        ...
        <tr v-for="(replys, index) in store.freeBoardReplyList" :key="replys.id" v-show="Math.ceil((index+1)/10) == pageNum" class="reply-detail">
      ...
      <p v-if="!store.freeBoardReplyList.length" style="color:rgba(0, 0, 0, 0.452); margin: 20px 80px; padding-bottom: 20px; border-bottom: 1px solid rgba(0, 0, 0, 0.13);">
        작성된 댓글이 없습니다.
      </p>
      <div class="page">
...
<script setup>
import {useFreeBoardReplyStore} from '@/stores/freeBoardReply'
import {useUserStore} from '@/stores/User'
import {onMounted, ref, computed} from 'vue'
import {RouterLink, useRouter, useRoute} from 'vue-router'

const store = useFreeBoardReplyStore()
...
onMounted(() => {
store.getFreeBoardReplyList(route.params.id)
})
...
const maxPageNum = computed(() => {
  return Math.ceil(store.freeBoardReplyList.length/10)
});
